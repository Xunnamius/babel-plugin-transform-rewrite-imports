...
pluginTester({
  pluginName: 'transform-rewrite-imports',
  plugin,
  babelOptions,
...
/* describe('Replace', () => {
  ...
  `(
    ...
    ({ statements, extension, replace, observedScriptExtensions }) => {
  ...
  `('should $type', ({ statements, extension, replace, observedScriptExtensions }) => {
    const { code } = babel.transformSync(statements, {
      plugins: [
        syntaxTypescript,
        [plugin, { extension, replace, observedScriptExtensions }]
      ],
      filename: ''
    });
